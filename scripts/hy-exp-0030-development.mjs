...
import { runHyExp0030Development } from '../src/research/hy-exp-0030.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const ARTIFACT_ROOT = path.join(ROOT, 'artifacts', 'HY-EXP-0030');
...
function writeJson(file, value) {
  ...
  if (fs.existsSync(file)) throw new Error(`refusing to overwrite locked HY-EXP-0030 artifact: ${relative(file)}`);
  fs.writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`, 'utf8');
}

function main() {
  const [command] = process.argv.slice(2);
  if (command !== 'run') throw new Error('usage: node scripts/hy-exp-0030-development.mjs run');
  if (fs.existsSync(RESULT_PATH) || fs.existsSync(CLOSURE_PATH)) {
    throw new Error('HY-EXP-0030 Development result is already locked; rerun refused');
  }
  const result = runHyExp0030Development({ root: ROOT });
  ...
  if (!result.experimentalReleaseReady) {
    ...
    const closure = {
      schemaVersion: 1,
      artifactType: 'HY_EXP_0030_CLOSURE',
      experimentId: 'HY-EXP-0030',
      ...
      nextStep: 'STOP_HY_EXP_0030; any new candidate family requires a new experiment ID.'
    };
    ...
    const event = appendRegistryEvent({
      ...
      note: 'HY-EXP-0030 VOLATILITY_COMPRESSION_EXPANSION Development gates failed; terminal paper-only closure, no prospective validation or Final OOS read.'
    });
