...
const PREREG_PATH = path.join(ROOT, 'registry', 'experiments', 'HY-EXP-0032', 'preregistration.json');
const MANIFEST_PATH = path.join(ROOT, 'artifacts', 'HY-EXP-0032', 'data-manifest.json');
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'HY-EXP-0032');
...
function verifyFrozenInputs() {
  if (manifest.preregistrationSha256 !== hash(fs.readFileSync(PREREG_PATH))) throw new Error('0032 preregistration hash mismatch');
  if (manifest.missingCount !== 0) throw new Error(`0032 data manifest missing ${manifest.missingCount} files`);
  if (manifest.outcomeRead || manifest.pnlComputed || manifest.finalOosRead) throw new Error('0032 data manifest is not pre-outcome clean');
...
function main() {
  ...
  const result = {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0032_STRATEGY_FAMILY_TOURNAMENT_RESULT',
    ...
    hyVal0032: { prepared: Boolean(winner), activated: false, experimentId: winner ? 'HY-VAL-0032-001' : null },
  ...
  const lines = [`# HY-EXP-0032 Strategy Family Tournament`, '', `Winner: **${result.winner}**`, '', '| Family | Events | Net18 bps | PF18 | Net27 bps | PF27 | Net36 bps | PF36 | MTM DD | CVaR | Gate |', '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|'];
