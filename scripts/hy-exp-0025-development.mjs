...
import { runHyExp0025Development } from '../src/research/hy-exp-0025.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'HY-EXP-0025');
...
function run() {
  for (const file of [MANIFEST, RESULT, TRADES, DIAGNOSTICS, CLOSURE]) {
    if (fs.existsSync(file)) throw new Error(`refusing to overwrite locked HY-EXP-0025 artifact: ${relative(file)}`);
  }
  const result = runHyExp0025Development({ root: ROOT });
  const manifest = {
  ...
  if (!result.experimentalReleaseReady) {
    ...
    failedEvent = appendRegistryEvent({
      ...
      note: 'HY-EXP-0025 Development fast gates failed; terminal paper-only closure, no live signal path implemented.'
    });
  }
...
try {
  if (process.argv[2] !== 'run') throw new Error('usage: node scripts/hy-exp-0025-development.mjs run');
  run();
} catch (error) {
