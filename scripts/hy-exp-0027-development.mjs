...
import { runHyExp0027Development } from '../src/research/hy-exp-0027.mjs';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', 'HY-EXP-0027');
...
function run() {
  for (const file of [MANIFEST, RESULT, TRADES, DIAGNOSTICS, CLOSURE]) {
    if (fs.existsSync(file)) throw new Error(`refusing to overwrite locked HY-EXP-0027 artifact: ${relative(file)}`);
  }
  const result = runHyExp0027Development({ root: ROOT });
  const manifest = {
    ...
    frequencyExposureStart: result.development.frequencyExposureWindow.start,
    frequencyExposureEndExclusive: result.development.frequencyExposureWindow.endExclusive,
  ...
  if (!result.experimentalReleaseReady) {
    ...
    failedEvent = appendRegistryEvent({
      ...
      note: 'HY-EXP-0027 Development diversified A-or-B rule gates failed; terminal paper-only closure, no Final OOS read or live path.'
    });
  }
...
try {
  if (process.argv[2] !== 'run') throw new Error('usage: node scripts/hy-exp-0027-development.mjs run');
  run();
} catch (error) {
