...
const ARTIFACT_DIR = path.join(ROOT, 'artifacts', HY_EXP_0038);
const PREREG_PATH = path.join(ROOT, 'registry', 'experiments', HY_EXP_0038, 'preregistration.json');
...
function runResearch() {
  ...
  if (dataManifest.experimentId !== HY_EXP_0038) throw new Error('DATA_MANIFEST_EXPERIMENT_MISMATCH');
  ...
  writeJson(path.join(ARTIFACT_DIR, 'development-oof-summary.json'), {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_DEVELOPMENT_OOF_SUMMARY',
    immutable: true,
    experimentId: HY_EXP_0038,
  ...
  const relativeArtifacts = [
    'registry/experiments/HY-EXP-0038/preregistration.json',
    'artifacts/HY-EXP-0038/data-manifest.json',
    'artifacts/HY-EXP-0038/development-report.json',
    'artifacts/HY-EXP-0038/frozen-model-spec.json',
    'artifacts/HY-EXP-0038/historical-validation.json',
    'artifacts/HY-EXP-0038/historical-validation.md',
    'artifacts/HY-EXP-0038/development-oof-summary.json',
    'artifacts/HY-EXP-0038/historical-trades.jsonl'
  ];
  if (emailPreparation) {
    relativeArtifacts.push('artifacts/HY-EXP-0038/email-preparation.json', 'registry/forward/HY-FWD-0037-001.json');
  }
...
else throw new Error('usage: node scripts/hy-exp-0038.mjs lock | run --code-commit FULL_SHA');
