...
function eventOptions(config) {
  return {
    ...
    momentumLookbackBars: config.features.momentum_lookback_bars,
    skipBars: config.features.skip_bars,
...
function featureOptions(config) {
  return {
    ...
    skipBars: config.features.skip_bars,
    betaLookbackBars: config.features.beta_lookback_bars,
    liquidityLookbackBars: config.features.liquidity_lookback_bars,
...
function screenThresholds(config) {
  return {
    ...
    maximumSymbolContributionShare:
      config.development_screen.maximum_positive_alt_symbol_contribution_share,
...
function writeReport(result) {
  const lines = [
    ...
    '',
    `固定日程产生 ${result.scheduledEvents} 个周组合，${result.tradedEventPortfolios} 个通过数据和容量规则。`,
    '',
...
function run() {
  ...
  const events = buildWeeklyEvents({ btcBars: btc, ...eventOptions(config) });
  ...
  for (const symbol of config.symbols) {
    ...
    for (const feature of features) {
      ...
      else candidatesByEvent.get(feature.eventId).push(feature);
    }
    coverage[symbol] = { contractBars: contract.length, features: features.length };
  }
  const attempts = events.map(event => buildMomentumPortfolio({
    event,
  ...
  for (const symbol of config.symbols) {
    ...
    coverage[symbol].fundingEvents = funding.length;
    coverage[symbol].markBars = mark.length;
  }
  const tradesByScenario = Object.fromEntries(scenarios.map(scenario => [
    scenario.name,
    portfolios.map(portfolio => executeMomentumPortfolio(portfolio, dataBySymbol, scenario))
  ]));
  ...
  const summaries = Object.fromEntries(Object.entries(tradesByScenario).map(([name, rows]) => [
    name,
    summarizeMomentumPortfolios(rows, altSymbols)
  ]));
  ...
  const result = {
    ...
    code: { runtime: process.version, command: 'npm run exp:012:run' },
    coverage,
...
try {
  ...
  else throw new Error('usage: exp012.mjs download | run | complete CODE_COMMIT');
} catch (error) {
