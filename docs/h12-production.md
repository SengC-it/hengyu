...
- Schedule: minute 5 of every fourth UTC hour via the Vercel `/api/h12-scan` cron in Singapore (`sin1`).
...
The former H9 workflow remains manually dispatchable for audit but no longer has a schedule. GitHub-hosted market-data execution is not used because Binance returns HTTP 451 from the US runner region.

Required Vercel secret: `CRON_SECRET`. The project must also retain its Supabase and Gmail variables. `HENGYU_INGEST_SECRET` is needed only for an explicitly triggered external/manual worker.
