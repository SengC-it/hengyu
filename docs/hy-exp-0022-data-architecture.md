...
## First prospective-bar smoke

`npm run hy-exp-0022:first-bar-smoke` is a separate engineering-only check for the frozen first bar `2026-08-22T04:00:00Z` through `2026-08-22T07:59:59.999Z`. It requires at least three dynamically selected symbols, a final contract-price WebSocket event and exact `startTime`/`endTime`/`limit=1` REST confirmation for each symbol, matching OHLCV/quote volume/trade count, `receivedAt` after the bar close, valid realized funding rows, and the complete current exchangeInfo filter schema. A transport-only or pre-capture observation can never satisfy this artifact. The artifact is `artifacts/HY-EXP-0022/first-prospective-bar-smoke.json`; its only successful state is `PASS`.
