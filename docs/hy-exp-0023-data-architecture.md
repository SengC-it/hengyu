...
Each depth connection is bounded to at most 20 dynamically selected symbols; the current 0023 profile uses batches of 20 for scalable combined streams. Kline connections use the same independent 20-symbol batch limit, so a 41-symbol set creates three kline connections and every batch has its own connection/reconnect segment. The 20 limit is per connection, never a total-universe limit. A depth segment is aligned per symbol. The first applied event must cover `lastUpdateId`; later events require `pu` to equal the previous `u`. A U/u/pu gap, duplicate, out-of-order update, crossed book, or invalid snapshot permanently invalidates that segment. A local receipt interval over the diagnostic limit is classified separately as `receipt_stall`: it still invalidates the segment and requires a fresh segment, but it is never reported as a source sequence gap when `pu === previous u`. Reconnects create a new segment and require a fresh REST snapshot; the invalid segment is never repaired. Receipt stalls, out-of-order receipt timestamps, native sequence gaps, duplicates and out-of-order updates have separate counters and detailed diagnostics.

The raw streams are `depth.diff`, `depth.snapshot`, `kline.4h`, `exchangeInfo`, `funding`, `universe.snapshot`, `universe.audit`, and `segment.audit`. Ticker is diagnostic-only and never substitutes for the six completed-bar quote-volume rule or decides all-eligible PIT membership. The all-eligible set uses only valid PIT exchangeInfo rules; ticker may rank a bounded engineering sample only, explicitly marked `SAMPLE_SELECTION_ONLY`. Funding requests use a bounded pool of eight, each request has an AbortController timeout, timeout aborts are counted in `fundingAbortedCount`, the peak is recorded in `fundingPeakInflightRequests`, and there is no retry. Missing/invalid rows, HTTP 429 or timeout are fail-closed. `st=2` is rejected, `st=1` is accepted when present, and raw `st`/`ps` are preserved.
...
Required health signals are collector death, sequence gap, crossed book, snapshot-alignment failure, missing receipt timestamp, receipt stall, stale data, NTP failure, manifest-sealing lag, and storage pressure. The supervisor reads a child progress heartbeat containing process id, segment id, last depth/kline/exchange timestamps, event count, written bytes and heartbeat time. A live process with no changing market-data progress is alerted as stale and restarted into a new segment. Alerts are append-only NDJSON, mirrored to stderr/logs, and readiness distinguishes a writable sink, configured alert types, verified runtime emitters and controlled fault-injection types. A writable alert file alone never makes `alertsActive=true`; supervisor fault-injection verification is required for formal readiness. OS clock synchronization and three Binance server-time samples (RTT, midpoint and absolute drift) are measured; timestamps are never rewritten.

Storage metrics record bytes/sec, events/sec, per-stream bytes, per-symbol rate, `initialBatchCount`, `activeConnectionBatchCount`, `websocketConnectionAttempts`, `reconnectCount`, `totalSegments`, `maxSymbolsPerConnection`, and projected 4-hour, 30-day, Development, Final-OOS, and full-experiment storage. Kline additionally records `klineInitialBatchCount`, `klineWebsocketConnectionAttempts`, `klineReconnectCount`, `klineMaxSymbolsPerConnection`, `klineSymbolsCaptured`, and `klineFailedBatchCount`; a failed batch is retained as an explicit failed segment even when a replacement connection captures later events. `totalSegments` is not a connection count: retries and boundary rotations are reported separately from the initial batch count. The capacity pilot measures the full currently eligible universe rather than extrapolating from a hidden total-symbol cap. For each segment, raw NDJSON is append-only; sealing requires close/fsync, SHA-256, immutable manifest, durable upload and remote checksum before any retention cleanup. A readiness pass requires durable storage, a retention plan, and available capacity at least equal to the projected full experiment. The local engineering dry-run filesystem is diagnostic evidence only and is not a production storage qualification; measured capacity shortfall remains a hard readiness blocker until an approved durable store is provisioned and evidenced.
