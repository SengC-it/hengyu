...
The raw streams are `depth.diff`, `depth.snapshot`, `kline.4h`, `exchangeInfo`, `funding`, `universe.snapshot`, `universe.audit`, and `segment.audit`. Ticker is diagnostic-only and never substitutes for the six completed-bar quote-volume rule or decides all-eligible PIT membership. The all-eligible set uses only valid PIT exchangeInfo rules; ticker may rank a bounded engineering sample only, explicitly marked `SAMPLE_SELECTION_ONLY`. Funding requests use bounded concurrency, preserve request/receipt timing, and fail closed on missing/invalid rows, HTTP 429 or timeout. `st=2` is rejected, `st=1` is accepted when present, and raw `st`/`ps` are preserved.
