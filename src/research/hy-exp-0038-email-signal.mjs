...
export const HY_EXP_0038 = 'HY-EXP-0038';
...
export function buildDataManifest({ root = ROOT, sourceManifest = loadSourceManifest({ root }) } = {}) {
  ...
  return {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_DATA_MANIFEST',
    immutable: true,
    experimentId: HY_EXP_0038,
  ...
  for (const symbol of FIXED_SYMBOLS) {
    ...
    for (let index = 96; index < s.contract15.length; index += 1) {
      ...
      for (const side of candidateSidesForRegime(regime.name)) {
        const base = {
          candidateId: `${HY_EXP_0038}:${symbol}:${side}:${decisionTime}`,
          experimentId: HY_EXP_0038,
          symbol,
          side,
          regime: regime.name,
...
export function runDevelopmentWalkForward(rows) {
  ...
  return { predictions, predictionsByLambda, folds };
}
...
export function selectDevelopmentConfig(predictionsByLambda, { series = null } = {}) {
  ...
  if (!MODEL_LAMBDAS.some(lambda => (sets[String(lambda)] ?? []).length)) {
    return { status: 'NO_DEVELOPMENT_CONFIG', reason: 'NO_OOF_PREDICTIONS', candidates: [] };
  }
  const percentiles = [];
  ...
  for (const lambda of MODEL_LAMBDAS) {
    ...
    if (!predictions.length) throw new Error(`INCOMPLETE_LAMBDA_OOF:${lambda}`);
    for (const percentile of percentiles) {
      ...
      if (accepted.length && rate >= 20 && rate <= 40 && expectancy > 0 && pf >= 1.10) {
        ...
        configs.push({ lambda, percentile, threshold, accepted, rate, expectancy, pf, trueMtmDrawdown });
      }
    }
  }
  ...
  configs.sort((left, right) => right.expectancy - left.expectancy || right.pf - left.pf
    ...
    || left.threshold - right.threshold || left.percentile - right.percentile || left.lambda - right.lambda);
  const best = configs[0];
  return {
    status: 'DEVELOPMENT_CONFIG_FOUND',
    lambda: best.lambda,
    edgeThresholdBps: best.threshold,
    selectedMinimumEdgeBps: best.threshold,
    NEGATIVE_MODEL_EDGE_THRESHOLD: best.threshold <= 0,
...
export function buildDevelopmentReport({ candidates, outcomes, walkForward, developmentConfig }) {
  ...
  return {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_DEVELOPMENT_REPORT',
    immutable: true,
    experimentId: HY_EXP_0038,
...
export function buildHistoricalValidationReport({ history, series, developmentConfig }) {
  ...
  const result = {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_HISTORICAL_VALIDATION',
    immutable: true,
    experimentId: HY_EXP_0038,
    window: {
...
export function buildFrozenModelSpec({ developmentConfig, preregistrationSha256 }) {
  return {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_FROZEN_MODEL_SPEC',
    immutable: true,
    experimentId: HY_EXP_0038,
...
export function buildEmailPreparation({ validation, codeCommit, preregistrationSha256, dataManifestSha256 }) {
  ...
  return {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_PAPER_EMAIL_PREPARATION',
    immutable: true,
    experimentId: HY_EXP_0038,
    codeCommit,
    preregistrationSha256,
...
export function buildCompletionBundle({ codeCommit, preregistrationSha256, dataManifestSha256, artifactEntries, finalResult, emailPreparation = null }) {
  ...
  return {
    schemaVersion: 1,
    artifactType: 'HY_EXP_0038_COMPLETION_BUNDLE',
    immutable: true,
    experiment_id: HY_EXP_0038,
    ...
    finalResult,
    artifacts,
    emailPreparation: emailPreparation ? 'artifacts/HY-EXP-0038/email-preparation.json' : null,
